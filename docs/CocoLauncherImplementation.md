...
### Experiencias Multimedia (`coco-media`): Series y Películas

Coco Launcher soporta dos canales de distribución multimedia bajo `runtime.type: "media"` y `launch.workflow: "coco-media"`:
1. **Series Episódicas (`content.type: "episodic-video"`):** Diseñado para contenidos con múltiples capítulos o partes divididas (como Heart Signal). Muestra un selector modal con la lista de partes, estado individual de cada una y botón de acceso `EPISODIOS`.
2. **Películas (`content.type: "movie"`):** Diseñado para largometrajes y piezas únicas (como *The Drama (2026)*). Presenta la tarjeta con el botón **`VER PELICULA`**, omitiendo cualquier diálogo de episodios e iniciando directamente la reproducción en el reproductor integrado WPF (`MediaElement`) mediante streaming HTTPS o copia local verificada en `%USERPROFILE%\Downloads\<downloadFolderName>`. Si no está descargada ni se transmite, gestiona la descarga reanudable con progreso.

Al abrir cualquier contenido multimedia, Coco prioriza `streamUrl` en el reproductor integrado. Si no hay `streamUrl` pero sí `sourceUrl`, ofrece una descarga reanudable en `%USERPROFILE%\Downloads\<downloadFolderName>`; el archivo local se verifica por tamaño y SHA-256 y no se duplica al reproducirlo. La ventana del reproductor usa una cabecera propia sin el marco clásico, controles sin bordes, barra de reproducción dibujada para Coco, volumen, pausa/reanudar, doble clic y F11/Escape para pantalla completa. El reproductor actual es `MediaElement` de WPF: no requiere VLC ni otro programa instalado y usa los códecs de Media Foundation disponibles en Windows. El caso objetivo H.264/AAC en Windows 10/11 normal queda cubierto.

La meta de publicación sigue siendo GitHub. Un GitHub Release no admite un asset individual de 2 GiB o más ([límite oficial de Release](https://docs.github.com/en/repositories/releasing-projects-on-github/about-releases)). *The Drama (2026)* mide 2,021,287,571 bytes (1.88 GiB), cumpliendo holgadamente con el límite de asset individual por debajo de 2 GiB.

La presencia en el catálogo publicado es la única regla de disponibilidad: toda entrada `managed` con workflow `coco-managed`, `coco-standalone` o `coco-media` aparece en Coco. Las experiencias Minecraft siguen la sesión única del host; una experiencia `coco-media` se puede abrir desde el cliente sin sesión Minecraft y sin que el launcher del host esté abierto. La red puede seguir preparándose para las experiencias Minecraft, pero no participa en el streaming multimedia. Las etiquetas `blocked`, `experimental`, `normal` y `validated` fueron eliminadas del schema y del engine. Las auditorías conservan evidencia y trabajo pendiente sin alterar visibilidad.
...
Rendimiento y robustez del instalador: el tamaño en disco por instancia y el mapa de ubicaciones se cachean con TTL corto (5s y 2s) y se invalidan al instalar, mover o eliminar, para no recorrer GB en el hilo UI en cada refresh de tarjetas. Los paquetes standalone multipart remotos adelantan hasta **3 partes simultáneas** con `curl.exe`; cada stream conserva reanudación por parcial, reintentos y corte de estancamiento (<1 KB/s por 90s), mientras la validación y extracción siguen el orden declarado para evitar escrituras rivales. El estado standalone nuevo registra también `archiveShas` en orden para fijar la identidad exacta de todas las partes. Después de que la ruta normal verifica por SHA-256 los archivos base instalados, los ZIP/partials bajo `downloads\standalone-packs` se eliminan: si más adelante hace falta reparar, se vuelve a descargar únicamente el archivo fijado necesario. Al abrir el launcher también se hace un barrido retroactivo: elimina instaladores de experiencias ya instaladas y residuos de versiones que ya no aparecen en el catálogo, pero conserva todas las partes de experiencias actuales incompletas, corruptas o cuyo estado multipart no coincide para poder reanudar/reparar sin perder progreso. Si dos experiencias comparten el mismo SHA, una instalación incompleta protege ese cache aunque la otra ya esté instalada. La validación de ZIP rechaza `saves/` y `playerdata`, igual que los overrides administrados. No se relee lo ya verificado en la misma ejecución: el hash final por parte se omite si la caché, el parcial reutilizado o el prefetch/descarga ya lo midieron, y la instalación managed reutiliza el resultado de la descarga paralela (conjunto verificado) en vez de re-hashear el pack y cada asset. Las 5 consultas WMI de procesos Java del updater comparten un snapshot de 3s con fallback al último conocido.

Robustez general: la espera de elevación de red tiene deadline de 15 minutos (sin matar msiexec a mitad); el servicio de sesiones exige deadline por solicitud, atoms en `owner.json` y valida la hora de inicio del padre contra reciclaje de PID; el updater verifica por CommandLine antes de terminar un PID y escribe `state.json`, `target.json` y el marcador de forma atómica; el parseo ZeroTier tolera prefijos no-JSON; la UI nunca crea tamaños negativos, limpia tarjetas huérfanas sin experiencias, no deja la bandera de instalación pegada, acota los cachés de imágenes a 64 entradas y parte palabras largas de títulos. En publicación: `Invoke-WithRetry` definido antes de usarse, bump a medio aplicar anterior bloquea (solo sus tres archivos), commit vacío validado, subidas en paralelo con limpieza idempotente de duplicados y fallo rápido, verificación de contenido por SHA en assets de hasta 100 MB, y chequeo de existencia real tras compilar VALORANTCraft.

Las experiencias standalone (Big Walk) reciben sus archivos adicionales por el mismo `experiences[].files`: un archivo cuyo `path` termina en `.zip` se descarga verificado y se extrae en la raíz de la instancia, como paquete de mods BepInEx, sin re-descargar las partes del juego. El estado `.coco\standalone-state.json` registra `filesSha` y un manifiesto de los archivos extraídos; Coco valida existencia, tamaño y SHA-256 al abrirse, y repara estados antiguos o archivos faltantes sin tocar mundos ni datos del jugador. Los paquetes deben excluir las carpetas generadas por la máquina al primer arranque (`interop`, `unity-libs`, `cache`) y los logs. Big Walk declara un paquete (2,6 MB) con BepInEx 6.0.0, Big Voice 1.0.0 (volumen VOIP por jugador, menú F10), Enhanced Controls 1.0.1 (auto-run, salto asistido, salida rápida) y Big Disrespect 1.0.1 (FlipOff); sus fuentes oficiales son Nexus (mods 5, 4 y 19) y la licencia declarada es All Rights Reserved.
...
Coco Launcher no desactiva la protección en tiempo real. Las experiencias standalone que declaran `runtimePolicies.defenderExclusion` usan exclusiones persistentes de carpeta mediante `Ensure-CocoDefenderExclusion`: se conserva la ruta estándar de Coco, las ubicaciones personalizadas registradas en `instance-locations.json` y las rutas heredadas necesarias para instalaciones existentes. `engine\CocoDefenderControl.ps1` se conserva únicamente como shim inerte porque bootstrappers antiguos exigen que el archivo exista; no contiene lógica de toggle. Las versiones antiguas que dejaron `CocoDefenderDisable`, `CocoDefenderEnable` o `%LOCALAPPDATA%\CocoMinecraftUpdater\tools\defender-control` se migran silenciosamente al iniciar el engine: las tareas se desregistran cuando Windows lo permite y los binarios locales se eliminan. Defender permanece activo durante launcher, instalación y juego. Detalles operativos: [OPERACION.md](OPERACION.md).

## Ventana de créditos online-fix

La ventana de créditos que online-fix muestra en el primer arranque de cada juego se suprime en dos capas. La preventiva escribe `[Hashes]` y el estado público del crack (`Ensure-CocoOnlineFixSuppression`). La universal es `CocoPopupGate` (`CocoLauncher.ps1`): un vigilante nativo activo durante toda la partida que recibe un evento del sistema en el mismo instante en que aparece una ventana del árbol completo de procesos del juego —no solo el PID lanzado— y la resuelve **ocultándola primero** para que nadie la vea ni pulse nada: pulsa su botón real (etiquetas inglés/español/ruso), acepta diálogos nativos o cierra variantes marcadas `online-fix`; nunca toca la ventana principal del juego. Si una ventana oculta no se resolvió en 6 segundos vuelve a hacerse visible como respaldo manual. Las preferencias `preferences.popupGate` (markers/buttonLabels/dialogClasses) permiten ajustes declarativos por experiencia sin ramas por nombre de pack; los candidatos no atendidos quedan registrados con prefijo `POPUPGATE`.
...
- El mismo sistema anti-corte rige para TODOS los textos del launcher (`Set-CocoFittedLabelText`/`Set-CocoFittedButtonText` en `engine\CocoLauncher.ps1`): cada etiqueta y botón mide su texto con `TextRenderer` sobre su rectángulo real en esa PC (DPI y fuentes reales) y baja la fuente hasta que cabe, sin superar el tamaño de diseño. Los botones de texto dinámico (reproductor, episodios, identidad, host) se pre-ajustan con su variante más larga o se reajustan en cada cambio de estado, por lo que un cambio de PC o DPI no puede recortar ni solapar textos.
