...
Rendimiento y robustez del instalador: el tamaño en disco por instancia y el mapa de ubicaciones se cachean con TTL corto (5s y 2s) y se invalidan al instalar, mover o eliminar, para no recorrer GB en el hilo UI en cada refresh de tarjetas. Las descargas standalone con `curl.exe` abortan si la transferencia se estanca (<1 KB/s por 90s, con reintento y parcial conservado) y su validación de ZIP rechaza `saves/` y `playerdata`, igual que los overrides administrados. No se relee lo ya verificado en la misma ejecución: el hash final por parte se omite si la caché, el parcial reutilizado o la descarga ya lo midieron, y la instalación managed reutiliza el resultado de la descarga paralela (conjunto verificado) en vez de re-hashear el pack y cada asset. Las 5 consultas WMI de procesos Java del updater comparten un snapshot de 3s con fallback al último conocido.
