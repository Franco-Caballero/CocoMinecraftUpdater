...
- **Autoaceptación limitada al launcher**: el host acepta nodos pendientes únicamente mientras su proceso Coco Launcher está abierto. El autorizador se liga al PID del launcher, consulta en vivo y se detiene cuando ese proceso termina; no instala tareas programadas ni sigue aceptando miembros con el launcher cerrado. Después de autorizar, republica la configuración de red para entregar IP de inmediato. El firewall sigue limitando la exposición a TCP/UDP 25565 y TCP 25564 desde `10.77.37.0/24`.
...
- Diagnóstico de autoaceptación: `%LOCALAPPDATA%\CocoMinecraftUpdater\logs\zerotier-authorizer.log` y `%LOCALAPPDATA%\CocoMinecraftUpdater\network\authorizer-<networkId>.json`. La tarea heredada `Coco ZeroTier AutoAccept` debe estar ausente.
...
- El host acepta nodos pendientes sólo mientras Coco Launcher permanece abierto.
...
- La autoaceptación funciona únicamente durante una sesión abierta de Coco Launcher. Con el launcher cerrado, los nodos nuevos quedan pendientes; revocar a alguien exige expulsarlo de la red.
