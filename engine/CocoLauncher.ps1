...
function Show-CocoUsernameDialog([string]$Suggested=''){
    ...
    $ok=New-Object Windows.Forms.Button;$ok.Text='Guardar';$ok.Location=New-Object Drawing.Point(235,118);$ok.Size=New-Object Drawing.Size(155,34);$ok.Add_Click({if(Test-CocoMinecraftUsername $input.Text){$dialog.Tag=$input.Text.Trim();$dialog.DialogResult=[Windows.Forms.DialogResult]::OK;$dialog.Close()}else{[Windows.Forms.MessageBox]::Show('Usa 3 a 16 letras, numeros o guion bajo.','Nombre invalido')|Out-Null}})
