...
if(-not$loveEp2-or$loveEp2.fileName-ne'Love.Island.US.S06E02.720p.Spanish.Hardsub.mp4'-or
   [int64]$loveEp2.size-ne1876473189-or$loveEp2.sha256-ne'ed4c9aa4eea153e658bdfa77b68b09aca6d72d79433d7a5d3bc8c33465fe09b7'-or
   $loveEp2.streamUrl-notmatch'^https://'-or$loveEp2.sourceUrl-notmatch'^https://'){
...
$rhythmDoctor=@($catalog.experiences|Where-Object id -eq 'rhythm-doctor'|Select-Object -First 1)[0]
if(-not$rhythmDoctor-or[string]$rhythmDoctor.runtime.executable-ne'Rhythm Doctor.exe'-or$rhythmDoctor.runtime.type-ne'standalone'-or$rhythmDoctor.managementMode-ne'managed'){
    throw 'La experiencia standalone Rhythm Doctor no esta declarada correctamente.'
}
$rdRequired=@($rhythmDoctor.runtime.requiredFiles)
if($rdRequired.Count-ne5-or`
   @($rdRequired|Where-Object{[string]$_.sha256-notmatch'^[a-f0-9]{64}$'-or[int64]$_.size-le0-or[string]$_.archiveSha256-notin@($rhythmDoctor.pack.archives.sha256)}).Count){
    throw 'Rhythm Doctor no fija todos sus archivos base reparables por ruta, hash, tamano y archive exacto.'
}
if([string]$rhythmDoctor.runtimePolicies.defenderExclusion-ne'required'-or[string]$rhythmDoctor.runtimePolicies.onlineFixAppId-ne'774181'){
    throw 'Rhythm Doctor no declara sus politicas standalone de Defender y OnlineFix.'
}
$moveOrDie=@($catalog.experiences|Where-Object id -eq 'move-or-die'|Select-Object -First 1)[0]
if(-not$moveOrDie-or[string]$moveOrDie.runtime.executable-ne'MoveOrDie.exe'-or$moveOrDie.runtime.type-ne'standalone'-or$moveOrDie.managementMode-ne'managed'){
    throw 'La experiencia standalone Move or Die no esta declarada correctamente.'
}
$modRequired=@($moveOrDie.runtime.requiredFiles)
if($modRequired.Count-ne5-or`
   @($modRequired|Where-Object{[string]$_.sha256-notmatch'^[a-f0-9]{64}$'-or[int64]$_.size-le0-or[string]$_.archiveSha256-notin@($moveOrDie.pack.archives.sha256)}).Count){
    throw 'Move or Die no fija todos sus archivos base reparables por ruta, hash, tamano y archive exacto.'
}
if([string]$moveOrDie.runtimePolicies.defenderExclusion-ne'required'-or[string]$moveOrDie.runtimePolicies.onlineFixAppId-ne'323850'){
    throw 'Move or Die no declara sus politicas standalone de Defender y OnlineFix.'
}
$wormsWmd=@($catalog.experiences|Where-Object id -eq 'worms-wmd'|Select-Object -First 1)[0]
if(-not$wormsWmd-or[string]$wormsWmd.runtime.executable-ne'Worms W.M.D.exe'-or$wormsWmd.runtime.type-ne'standalone'-or$wormsWmd.managementMode-ne'managed'){
    throw 'La experiencia standalone Worms W.M.D no esta declarada correctamente.'
}
$wwmdRequired=@($wormsWmd.runtime.requiredFiles)
if($wwmdRequired.Count-ne5-or`
   @($wwmdRequired|Where-Object{[string]$_.sha256-notmatch'^[a-f0-9]{64}$'-or[int64]$_.size-le0-or[string]$_.archiveSha256-notin@($wormsWmd.pack.archives.sha256)}).Count){
    throw 'Worms W.M.D no fija todos sus archivos base reparables por ruta, hash, tamano y archive exacto.'
}
if([string]$wormsWmd.runtimePolicies.defenderExclusion-ne'required'-or[string]$wormsWmd.runtimePolicies.onlineFixAppId-ne'327030'){
    throw 'Worms W.M.D no declara sus politicas standalone de Defender y OnlineFix.'
}
$mimicParty=@($catalog.experiences|Where-Object id -eq 'mimic-party'|Select-Object -First 1)[0]
if(-not$mimicParty-or[string]$mimicParty.runtime.executable-ne'Mimic Party.exe'-or$mimicParty.runtime.type-ne'standalone'-or$mimicParty.managementMode-ne'managed'){
    throw 'La experiencia standalone Mimic Party no esta declarada correctamente.'
}
$mpPartyRequired=@($mimicParty.runtime.requiredFiles)
if($mpPartyRequired.Count-ne5-or`
   @($mpPartyRequired|Where-Object{[string]$_.sha256-notmatch'^[a-f0-9]{64}$'-or[int64]$_.size-le0-or[string]$_.archiveSha256-notin@($mimicParty.pack.archives.sha256)}).Count){
    throw 'Mimic Party no fija todos sus archivos base reparables por ruta, hash, tamano y archive exacto.'
}
if([string]$mimicParty.runtimePolicies.defenderExclusion-ne'required'-or[string]$mimicParty.runtimePolicies.onlineFixAppId-ne'5053820'){
    throw 'Mimic Party no declara sus politicas standalone de Defender y OnlineFix.'
}
$theCabin=@($catalog.experiences|Where-Object id -eq 'the-cabin'|Select-Object -First 1)[0]
if(-not$theCabin-or[string]$theCabin.runtime.executable-ne'the_cabin_game.exe'-or$theCabin.runtime.type-ne'standalone'-or$theCabin.managementMode-ne'managed'){
    throw 'La experiencia standalone The Cabin no esta declarada correctamente.'
}
$tcRequired=@($theCabin.runtime.requiredFiles)
if($tcRequired.Count-ne6-or`
   @($tcRequired|Where-Object{[string]$_.sha256-notmatch'^[a-f0-9]{64}$'-or[int64]$_.size-le0-or[string]$_.archiveSha256-notin@($theCabin.pack.archives.sha256)}).Count){
    throw 'The Cabin no fija todos sus archivos base reparables por ruta, hash, tamano y archive exacto.'
}
if([string]$theCabin.runtimePolicies.defenderExclusion-ne'required'-or[string]$theCabin.runtimePolicies.onlineFixAppId-ne'4406280'){
    throw 'The Cabin no declara sus politicas standalone de Defender y OnlineFix.'
}
$ghostJanitors=@($catalog.experiences|Where-Object id -eq 'ghost-janitors'|Select-Object -First 1)[0]
if(-not$ghostJanitors-or[string]$ghostJanitors.runtime.executable-ne'Ghost Janitors.exe'-or$ghostJanitors.runtime.type-ne'standalone'-or$ghostJanitors.managementMode-ne'managed'){
    throw 'La experiencia standalone Ghost Janitors no esta declarada correctamente.'
}
$gjRequired=@($ghostJanitors.runtime.requiredFiles)
if($gjRequired.Count-ne6-or`
   @($gjRequired|Where-Object{[string]$_.sha256-notmatch'^[a-f0-9]{64}$'-or[int64]$_.size-le0-or[string]$_.archiveSha256-notin@($ghostJanitors.pack.archives.sha256)}).Count){
    throw 'Ghost Janitors no fija todos sus archivos base reparables por ruta, hash, tamano y archive exacto.'
}
