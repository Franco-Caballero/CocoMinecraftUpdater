...
try{
    ...
    if($LASTEXITCODE -ne 0){throw "Instalación de red terminó con código $LASTEXITCODE"}
    ...
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $engine -ManifestPath $manifest -GameDir $game -Silent
    $watch.Stop()
    if($LASTEXITCODE -ne 0 -or $watch.Elapsed.TotalSeconds -gt 5){throw 'La segunda verificación no reutilizó la instalación actual.'}
    'PASS: descarga GitHub, SHA-256, instalación exacta y reutilización.'
}finally{
